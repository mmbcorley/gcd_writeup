...
Why then do listeners hold such strong associations and beliefs between visual cues and lying?\todo{something went wrong with this sentence.  And are the beliefs `strong'?}\todo{JK fix}
...
The heuristics may be predicated on lay beliefs about deception, or on introspection as a speaker rather than experience as a listener, but ultimately lead to the somewhat innaccurate model of what deceitful behaviour looks and sounds like. \todo{needs fleshed out a bit?}\todo{JK}
...
\todo{JK. I know. i'm not sure what the point of experiment 2 is any more}
...
Visual stimuli consisted of the same 120 line drawings from \citet{Snodgrass1980} which were used in \citet{Loy2017} sixty of which served as referents and the other sixty as distractors.
Referents were randomly paired with distractors and presented across sixty critical trials (rather than the 20/40 critical/filler split in \citealt{Loy2017}). 
%JK reword.
...
These ten videos were each seen in three trials, resulting in 30 trials presenting a video including no cue.
The remaining 25 videos of the speaker included one of three types of cue.
Five videos of trunk movements were each presented in two trials each; ten videos of adaptor gestures (e.g. finger tapping, head tilts) were each presented in one trial each; and ten videos of the speaker sitting motionless but in a different posture to that of the no-cue videos (e.g. hand on chin, arms crossed, etc) were each presented in one trial each.
This meant that each participant saw 30 videos with no-cue, and 30 videos with a cue (10 showing trunk movements; 10 showing adaptor gestures; 10 showing different postures).
...
For videos showing a trunk movement, this was the frame at which the movement ended, meaning that participants were presented with no overlap between cue and speech. 
...
The frame numbers for trunk movements and adaptor gestures were matched by videos showing a motionless speaker, thus controlling for any sensitivity to the duration of video prior to speech (which could be interpreted as speech-initiation time, in turn a potential cue to deception).\todo{JK i think this is misleading. we matched adaptor gestures with different posture movements, and trunk movements to no-cue}

As with \citet{Loy2017}, 20 referents were counterbalanced across two lists. 
Each list contained 10 videos showing a trunk movement and 10 videos showing no cue..
...
The remaining 40 referents were randomly paired with one of the remaining videos (10 showing adaptor gestures, 10 showing different postures, 20 showing no-cue) for each participant, with no repetition of referents across videos.\todo{JK: So, not properly counterbalanced, but that was because we were really just looking at trunk movements}


\subsection{Procedure}
Stimuli were displayed on a 21~in.\@ CRT monitor with a resolution of 1024x768, placed 850~mm from an Eyelink~1000 Tower-mounted eye-tracker which tracked eye movements at 500~Hz (right eye only). 
...
This was replaced by the two images of the referent and distractor, measuring 150x150 pixels, centered vertically and positioned such that the center of each object was 15\% from either edge of the display. 
This display lasted for 2000ms before the video was added and the cursor was centred and made visible.
The video, measuring 336x236 pixels, was displayed with the bottom edge at the middle point vertically, and centered horizontally.
...
25\% of trials (half presenting a video with some sort of cue; half presenting a video with no-cue) were randomly designated as bonus rounds for each participant.
...
Trials in which participants did not click on either the referent or distractor (0.003\% of trials)\todo{is that a single trial? JK I believe it's 4 trials} were excluded from all analyses. 
...
Reaction times (measured from referent onset) were modelled with the same fixed and random effect structure, using mixed effects logistic regression models with an inverse gaussian distribution \citep[See][]{Lo2015}.
