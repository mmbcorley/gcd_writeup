...
Here, we investigate the integration of visual information about a speaker into judgements of deception, and ask if and when associations between nonverbal cues and deception emerge.
%By studying the time course of judgements of a speaker's (dis)honesty when presented with different visual cues to deception, we ask whether addressees are relying upon rule-of-thumb associations between visual cues and deception, or whether a link between gestures and perceived deception requires a more complex inferential process.
...
Experiment~1 investigated addressees' associations between visual cues and deception, using a variety of static and dynamic cues.
Experiment~2 focused on adaptor gestures. 
We show that a speaker's nonverbal behaviour can have a rapid and direct influence on addressees' pragmatic judgements, supporting the idea that communication is fundamentally multimodal.
...
Nonverbal behaviours, however, are substantially more varied than speech interruptions, serving as both potential markers of metacognitive states and planning processes, and as an alternative modality in which a speaker can convey semantic information \citep[See, e.g.][]{Ekman1969,Mcneill1992}. %% jia: should be made more explicit that we're talking about the role of nonverbal behaviour from the listener's perspetive (since this is the focus here), rather than the speaker's
Any rule-of-thumb linking visual information to deception would have to be refined enough to discriminate types and contents of nonverbal behaviours, or risk potentially over-attributing any cue as a sign of deceit. %% jia: also wasn't sure what 'content' of nonverbal behaviour refers to sorry!
...
An alternative to the heuristic explanation suggests that non-linguistic information might influence listeners' judgements of deception via a form of on-the-fly speaker modelling: %% jia: not quite sure I like 'on-the-fly' (though it did make me laugh), but will come back to it later
...
%% jia: sentence here summarising the focus of the study: In the current study, we investigate <unfolding of listeners' deception judgements based on a speaker's nonvebal behavioural cues during on-line processing; very badly worded rephrase as appropriate>.
...
%%%%%%%%
%% jia, overall comments: I think the introduction is pretty good, main thing is that it was only around the second half that I felt like I had a better sense of what the study is interested in. i think i got derailed slightly by the second para which goes off on a slight tangent about how listeners may be wrong about the actual cues that speakers produce - i'm not sure that's hugely relevant; i think it'd be more useful to go from the first para to something like 'when and how do listeners integrate these visual cues during their judgements of deception? well we don't know, but here's some evidence that people integrate information in the auditory channel really quickly. however, the visual channel is potentially more complex because x,y,z (i think you talked about this at some point), so that's why it's interesting to us.' another thing that i thought would be useful is some cites on the time course of gesture comprehension outside of deception.. i feel like there must be some literature on this, and would definitely be relevant to cite. i'd try and work in a few sentences of that.
%%%%%%%%%
...
%% jia: next few sentences are probably a bit too wordy and hard for a reader who isn't familiar with the design to wrap their head round (i had to think about it for a while before I remembered why we even cared about having them occur at the start of the utterance). Testing out a rewrite which hopefully simplifies things but still gets at what you're trying to convey:
% We focus on trunk movements based on previous research indicating that listeners perceive these movements as cues to lying \citep{Vrij1996a}, as well as their plausibility as an utterance-initial gesture, allowing us to avoid potential confounds due to an overlap in presentation of the critical visual and auditory information in a visual world paradigm.
% Based on a post-hoc analysis on filler trials which suggested that listeners' judgements were most strongly influenced by the speaker's adaptor gestures, we designed Experiment~2 to explore how this cue influences judgements of veracity.
...
%% jia: Just a (general) note which I'm going to make here in case I forget to at the end - you seem to fluctuate between judgements of veracity and deception/falsehood. i know they're essentially binary but probably better to just pick one and stick to it
...
The remaining 20 videos (each presented in one filler trial) showed the speaker either producing an adaptor gesture, such as finger-tapping or head-tilting, (10 videos) or sitting motionless but in a different, non-neutral posture, such as hand-on-chin or arms crossed (10 videos). %% jia: maybe talk about critical and filler trials separately otherwise it's slightly confusing - just say 20 critical trials: the 10 motionless were used as 'still' videos, 5 of trunk movements were used as 'gesture' videos (each x2); 40 filler trials: same 10 from critical trials were used as 'still' videos (each x2), 20 other gesture videos of either adaptors (x10) or motionless but in a different posture (x10).
This meant that over the course of the experiment each participant saw 30 videos of the speaker producing no cue, and 30 videos of the speaker producing some type of cue (10 showing trunk movements; 10 showing adaptor gestures; 10 showing different postures). %% 'a potential cue to deception' rather than 'some type of cue'
...
The trial ended once the participant clicked on either object, or 5000~ms after the onset of the referent noun. %% jia: -> or timed-out 5000~ms after onset of the referent noun, at which point participants saw a message telling them to click on an object faster
...
Participants were told that the speakers in the previous experiment would lie approximately half of the time. %% jia: 'had lied'.. since it was purportedly in the past? actually i'm not sure because current expt is in the present, but had lied just sounds better to me
...
%% jia: did we ever extend the time window for the critical trial analysis to 1100 ms at all to see if there were any on-line effects? just occurred to me that one difference between the critical and filler analyses is the time window duration, and it seems reasonable to ask whether the effects we observed in the filler trials were just due to the extended time window
...
Overall, these findings contrast with our prediction that trunk movements would provide the best opportunity to investigate the association between nonverbal behaviour and lying, and suggest that other visual cues may elicit more judgements of deception. %% jia: i'm not sure we can actually say that our prediction was that trunk movements would provide the best opportunity since it wasn't part of our design to do a cue comparison, maybe just say something like 'these findings suggest that, counter to our prediction [intuition/hypothesis/not sure of a good word?] that trunk movements would be associated with lying, listeners may in fact rely on other visual cues when making judgements of deception
% also again - standardise whether we say judgements of deception/honesty/veracity etc, think there are too many terms going on now
One possible explanation for why participants did not reliably associate trunk movements with deception could be that, in these trials, the audiovisual information immediately available to the listener at the critical point on each trial (the time of referent onset) was comparable to the videos showing no cue (trunk movements having ended at speech onset).
...
This explanation, however, is at odds with the bias towards judgements of deception following utterance-initial disfluencies observed in previous work \citep[][Experiment~1]{Loy2017}. %% jia: i'm not sure this point makes sense (or is necessary), since the disfluency study also found an effect of utterance medial pauses, so it's not as though they argue that utterance initial but not medial cues have an effect compared to our finding here that utterance medial but not initial cues have an effect. if you want to bring it up we should at least add something along the lines of 'suggesting that utterance-initial nonverbal cues may not be interpreted similarly to utterance-initial speech cues in the context of judging deception <reword as necessary>', to clarify that we're just making a comparison of utterance initial cues here
...
%JK is this reasonable? I'm saying that trunk movements may have been ignored because people attended more to the hand movements
%% jia: not 100% sure i'm understanding this sentence correctly - do you mean in the context of this experiment where the speaker produced a range of potential cues, listeners may have attended more to hand movements (for whatever reason)?
...
We manipulated the presentation of different potential nonverbal cues to deception while measuring eye  and mouse  movements of the listener towards one of two possible final judgements about the veracity of an utterance.
...
This is partly supported by studies which found a facilitative effect of illustrative gesturing on addressees' comprehension to be weakened for speakers who produce a lot of other, non-communicative movements \citep{Holle2007}. %% jia: seems reasonable and i like this point as it strengthens our basis for exp 2, but it's worth also mentioning that the vrij study's results were based on a questionnaire rather than empirical evidence, otherwise it would probably raise the question of whether they likewise had a bunch of cues and if so, why they found an association for trunk movements whilst we didn't
...
However, the lack of a reliable association between trunk movements and judgements of dishonesty suggests that care should be taken when generalising from peoples' beliefs about cues to deception \citep[e.g.][]{Vrij1996a, Zuckerman1981} to situations in which they are faced with a variety of possible cues. \todo{Is it also worth noting that adaptors were the only one of 4 visual cues in Zuckerman1981 not associated with perceived deception?}

It is worth noting that these studies show the possible benefits to be gained in extending the visual world paradigm to include visual information about the speaker, and not just the extensional world.
...
This is in keeping with suggestions that listeners extracting information about gestures through peripheral vision \todo{JK, also, fixations to video weren't low.. but then gullberg says people tend to look at the face}\citep[See e.g.][]{Gullberg2006}, although further research is needed to see whether a similar paradigm would work with gestures which represent ideational content.  
...
%JK not sure about gesture here.

%% jia: the GD needs to include something about the whole rule-of-thumb vs. speaker-modelling hypotheses, since we set that up in the introduction. currently it's framed as though we're mainly investigating the time course of gesture integration during pragmatic inferences about a speaker's veracity, which in itself is fine, but if so we then need to reframe how things are set up in the introduction. basically there's a bit of a mismatch, i think it'd be easier to just work in a para about the two hypotheses somewhere in the GD, probably around para 5, to address this point, rather than redo the intro
