...
% Jia: table caption probably needs a bit more information than just object clicks
\begin{table}
...
Interestingly, this early effect was not found for videos of trunk movements. % Jia: this was quite a long sentence so splitting it into three 
...
This meant that at the point of referent onset, the audiovisual information immediately available to the listener was comparable to the no gesture videos. % Jia: not sure we can really say they were indiscernible from, since technically they would still have looked slightly different from the still videos..
...
In contrast to previous versions of this paradigm, when presented with a possible cue to deception, the point at which listeners preference toward the referent tended to be matched (or overtaken, in the case of adaptor gestures) by the distractor was later, approximately 1000~ms post-referent onset. % Jia: I can't really parse this sentence, sorry! Can you rephrase it?
...
Firstly, the inclusion of longer referents, along with the fact that not all referent-distractor pairs were controlled for phoneme onset, may have resulted in establishing reference being delayed. % Jia: changing for tense consistency with the rest of the discussion
Secondly, the inclusion of videos in the experimental stimuli may have detracted from participants' fixations to the two objects. %visual component of the paradigm may be detracting from fixations to the two objects. % Jia: techically the objects are a visual component, so clarifying that it is the videos specifically
This could also explain the differences in the early window (0 to 800~ms) between the types of gesturing: The reduction in the referent-bias may be a result of the video being more salient in both the different posture and the adaptor gesture conditions, and not evidence of the emerging gesture-lying bias. % Jia: I'm confused by why we're referencing differences in a 0--800ms window here - the results don't mention any analysis on this time window
...
This is perhaps because the link from gestures to perceived deception may be subject to different processing mechanisms than the link from disfluency to deception %might follow a different path than that from disfluency to deception. % Jia: follow a different path sounded slightly vague to me
...
Additionally, we included a task after the eye-tracking segment asking participants to rate each video for how nervous they perceived the speaker to be. %and in which, after completing the eye-tracking study, participants rated the videos on perceived anxiety. % Jia: it's probably a minor point but i think we should be consistent with whether we call it nervousness/anxiety here and in the next section - was the experiment itself rating for 'nervousness'?

\section{Experiment 2}
% Jia: have expanded this section slightly because i think we need to give more info
...
We focused on adaptor gestures since these were found to have the largest effect on listeners' judgments of dishonesty in Experiment 1. % Jia: does this sound reasonable? (i thought we should at least briefly explain why we chose adaptors)
...
Using the same paradigm as Experiment 1, participants in Experiment 2 heard utterances accompanied by a video of a speaker either producing an adaptor gesture or no gesture, and were tasked with making an implicit judgement on whether the speaker was lying or telling the truth. % Jia: don't think you can see utterances :p
...
% Jia: we're currently switching back and forth between static and no-gesture and static/no-gesture, so i'm going to standardise and just call it no-gesture
...
This bias is in line with results from previous research on deception, which shows that listeners have a general tendency to judge a speaker as truthful (\citealt{Vrij2000}). % Jia: moving the truth-bias cite up here since it's more relevant
...
%is consistent with previous research in detecting actual lies from truths, which suggest a general tendency to judge a speaker as truthful (\citealt{Vrij2000}).

Consistent with previous research in deception judgments and beliefs about cues to deception, participants linked an increase in movement with dishonesty. % Jia: maybe add a reference here? i know we've cited this point somewhere in the backgroundy bit but it's quite distant by now and readers probably don't still have it in mind
% Jia: i moved this next bit from the last para up here because i think a. it follows on more directly from the final judgements and allows us to transition quite nicely from off-line to on-line, and b. i quite like how the discussion then ends on a 'future work could look at...' note - see if it works for you, if not feel free to switch it back
...
% Jia: i think we should try and say a bit more about this point (highlight that integration of the visual channel has a rapid and direct effect on pragmatic judgements, what implications this may have on comprehension in multimodal communication etc.), since it is afterall our main 'exciting' result
...
In Experiment 1, a reduction in the tendency to believe the speaker was apparent when the speaker was either in a different posture or made a trunk movement prior to speaking, although these resulted in no final tendency towards interpretations of either honesty or dishonesty. % Jia: i think the first and second parts of this sentence directly contradict each other.. do you mean a numerical difference but no significant effect?
...
Experiment 2 provided a clearer account of the influence of adaptor gestures on deception judgments at the early moments of referent comprehension, with the tendency to look --- and move the mouse --- towards the referent being largely attenuated when the utterance was presented with a gesture. % Jia: don't think we can say that the bias was *completely* attenuated
However, following adaptor gestures in both experiments, the bias towards the distractor --- signifying perceived dishonesty --- over the referent appeared approximately 1000~ms after the referent began, at a later point than previous versions of this paradigm in which speech disfluencies were found to modulate judgements of deception %have found speech disfluency to modulate deception judgments. % Jia: sorry, rephrased because i ran into issues parsing
%This may be simply due to listeners associating lying with \textit{any} disfluency, only \textit{some} gestures. % Jia: ah, sorry, i'm probably being really slow here but i couldn't really figure out how this sentence followed on from 
...
To better understand how cues to deception in different modalities affect comprehension, further research would require investigating the effect of disfluency when the visual channel is also available --- for example, studying the time course of deception judgments when faced with one or both of a disfluency and an adaptor gesture. %i.e. GVD % Jia: nice - i like the setting up for GVD

%Interestingly, participant's subsequent ratings of how nervous the speaker looked in each video did not pattern with their final judgments beyond whether or not the video presented a gesture, suggesting that listeners' associations between gesture and lying is not necessarily a consequence of perceived anxiety in the speaker.
%Although the mechanism behind listeners' associations between gestures and deception remain unclear, our results show that gestural cues influence pragmatic judgments at the early stages of comprehension, but suggest that .
